...
#### 新機能追加
* Kubernetes v1.35.5 をサポートします。

<a id="aug-25-2026-platform-version-updates"></a>
#### プラットフォームバージョンの更新
* 1.202608.0 追加
    * Kubernetes 互換バージョン: v1.31–v1.35
...
* Kubernetes v1.34.3をサポートします。
    * k8s v1.34以降のバージョンでは、ワーカーノードイメージのCGroupバージョンに応じた制約事項が存在します。詳細については、[Kubernetesバージョンとタイムバージョンに応じた制約事項](/Container/NKS/ja/version-guide/#constraints-on-cgroup)を参照してください。
    * k8s v1.34以降のバージョンでは、ユーザー定義のcontainerdレジストリ設定機能が動作しません。詳細については、[ユーザー定義のcontainerdレジストリ設定機能](/Container/NKS/ja/version-guide/#containerd-registry-config)を参照してください。
* Cilium CNIをサポートします。

<a id="may-27-2026-platform-version-updates"></a>
#### プラットフォームバージョンの更新
* 1.202605.0 追加
    * Kubernetes 互換バージョン: v1.30–v1.34
    * 機能追加
        * ワーカーノードの CGroup v1 → v2 マイグレーションのサポート
        * kube-apiserver と Pod 間の通信のための konnectivity のサポート
        * k8s v1.34 クラスターの `ImageVolume` feature gate の有効化
...
#### プラットフォームバージョンの更新
* 1.202602.0 追加
    * Kubernetes 互換バージョン: v1.29–v1.33
    * 機能追加
        * Kubernetes テイント設定機能
        * Kubernetes コンポーネント設定機能 max-pods 設定のサポート
        * Secure Key Manager を利用した etcd データ暗号化のサポート
        * CGroup v2 OS イメージのサポート
    * 機能改善
...
* Kubernetes コンポーネント設定機能により、kubelet の --max-pods を設定できます。
...
* ロードバランサー詳細オプション設定にヘルスチェックポート設定機能が追加されました。
* ロードバランサー詳細オプション設定にヘルスチェックホストヘッダー設定機能が追加されました。
...
#### 新規機能追加
* Kubernetes v1.32.3 をサポートします。
* コントロールプレーンの Kubernetes コンポーネントログ保存機能が追加されました。
* 指標ベースのオートスケーラー機能が追加されました。
* ノードグループごとのフローティング IP 自動割り当て機能を使用できます。
* ノードグループごとの Kubernetes ラベル設定機能を使用できます。
* Addon 管理機能を使用できます。
...
#### 新規機能追加
* kubelet ユーザー定義引数の設定機能が追加されました。
* ロードバランサーの詳細オプション設定として、メンバーサブネット設定機能が追加されました。
* ロードバランサーの詳細オプション設定として、keep-alive タイムアウト値の設定機能が追加されました。
* 暗号化されたブロックストレージを使用して PV を作成する機能が追加されました。
