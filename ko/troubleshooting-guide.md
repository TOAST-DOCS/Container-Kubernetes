...
## > 워커 노드의 컨테이너 로그 파일 크기가 계속 커지면서 디스크 공간이 줄어들고 있습니다.
### 로그 로테이션 설정하기
...
> [참고] `CentOS 7.8 - Container (2021.07.22)` 이후의 인스턴스 이미지에는 위와 같은 내용의 로그 로테이션 설정이 기본으로 제공됩니다.
<br>
...
아래는 ssh를 기반으로 클러스터를 구성하는 모든 워커 노드에 대해 로그 로테이션 설정 파일을 비교 후, 필요한 노드에 복사해 주는 스크립트를 생성해 주는 쉘 커맨드입니다.
