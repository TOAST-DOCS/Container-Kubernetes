...
### > NKS가 배포하는 필수 리소스에 finalizers 설정을 하면 마지막 노드 그룹의 클러스터 업그레이드가 실패합니다.
NKS가 배포하는 필수 리소스에 finalizers 설정이 적용되어 있는 경우, 리소스가 정상적으로 제거되지 않아서 발생하는 문제입니다. 노드 그룹을 순차적으로 업그레이드 진행했을 때 마지막 노드 그룹의 업그레이드가 완료되면 NKS의 필수 리소스가 재배포 됩니다. 이때 필수 리소스에 finalizers 설정이 적용되어있다면 제거에 실패하여 업그레이드이 중단됩니다. 이러한 문제를 해결하기 위해서는 관련 리소스에서 finalizers 설정을 제거해야 합니다.
