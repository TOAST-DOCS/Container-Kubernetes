...
| v1.29.3 | 2024.05 | 2026.02.28 (예정) | 2026.02.28 (예정) |
...
| 1.202602.0 | 2026.02 | v1.29–v1.34 | 기능 추가<br>- 노드 그룹 별 Taint 설정 기능<br>- 노드 그룹 별 최대 pod 수 설정 기능 <br>- SecureKeyManager를 이용한 etcd 데이터 암호화<br>- CGroup v2 OS 이미지 지원<br>- Cilium CNI 지원<br><br>기능 개선<br>- 노드 및 노드 그룹 삭제 시 로드 밸런서 트래픽 유실 개선 |

<a id="k8s-version-changes"></a>
## Kubernetes 버전별 주요 변경 사항

### v1.34 주요 변경 사항
Kubernetes v1.34 버전은 클러스터 데이터 저장소인 etcd의 버전을 상위 버전으로 변경하여 제공합니다. 이에 따라 v1.33에서 v1.34로 업그레이드를 진행할 경우, etcd 버전 업데이트 작업이 동반됩니다. 이 작업은 Control Plane의 재시작을 포함하므로 kube-apiserver가 일시적으로 중단될 수 있습니다. 업그레이드 계획 수립 시 아래의 서비스 영향을 참고하시기 바랍니다.
- API 호출 제한: 작업 진행 중 kubectl 명령어 사용 및 API 호출이 실패할 수 있습니다.
- 서비스 영향: 이미 실행 중인 Pod(Data Plane)의 통신에는 영향이 없으나, 신규 Pod 생성, 배포 및 스케일링은 제한됩니다.
- 권고 사항: 안정적인 운영을 위해 트래픽이 적은 시간대에 업그레이드를 진행하는 것을 권장합니다.
